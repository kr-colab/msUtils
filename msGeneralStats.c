...
//fourGametes-- returns 1 or 0 based on if pair of sites passes four gamete test
// for sites i and j
int fourGametes(int i, int j, int nsam, char** list){
	int x00, x10, x01, x11, k;
	x00 = x10 =  x01 = x11 = 0;
	...
	for(k=0; k<nsam; k++){
		if(list[k][i] != 'N' && list[k][j] != 'N'){
			...
			if(list[k][i] == '1' && list[k][j] == '1')
				x11 = 1;
			if(list[k][i] == '0' && list[k][j] == '0')
				x00 = 1;
			if(list[k][i] == '0' && list[k][j] == '1')
				x01 = 1;
			if(list[k][i] == '1' && list[k][j] == '0')
				x10 = 1;	
		}
	}
	if(x00+x11+x01+x10 == 4){
		return(1);
	}
	else{
		return(0);
...
//performs the fourGameteTest on all pairs of sites. returns 1 or zero if 4Gams found somewhere or not
int fourGameteTest(int segsites, int nsam, char **list){
	...
	if(segsites < 2){
		...
		for(i=0; i<segsites-1; i++){
			for(j=i+1; j<segsites; j++){
				count += fourGametes(i,j,nsam,list);
			}
		}
	}
	if(count>0) return(1);
