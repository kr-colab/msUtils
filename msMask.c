/******* ms2SFS.c ********
just converts ms output to sfs style input
********************************/
...
/* read in a sample */
	...
	//	fgets( line, LINEBUF, pfin);
		...
		//read the next mask
		readNextMask(maskFile,aMask, nsam);
		if( segsites > 0) {
			//print positions
			fgets( line, LINEBUF, pfin);
			printf("%s",line);
			...
			for(i=0; i<segsites ;i++){
				for(j=0;j<nsam;j++){
					//printf("%d\n",(int)(posit[i]*LENGTH));
					if(aMask->bitmap[j][(int)(posit[i]*LENGTH)] == 0){
						list[j][i] = 'N';
						}
				}
			}
